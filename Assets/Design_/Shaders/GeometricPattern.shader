Shader "Custom/GeometricPattern"
{
	Properties
	{
		_OffsetX("OffsetX",Range(0,4)) = 1
		_OffsetY("OffsetY",Range(0,4)) = 1
		_OffsetZ("OffsetZ",Range(0,4)) = 1
		...
		SubShader
	{
	   ...
	   Pass{
			...
			rm RayMarching(float3 pos) {
				...
				//o.dis = (length(max(abs(p.xyz) - float3(1, 1, 1), 0.0)) - _Rounded) / p.w; //最後の引き算は丸みをつけるため
				o.dis = (length(p.xy)-0.5) / p.w;
