	...
	Properties
	{
	   ...
	   _MainColor("MainColor",Color) = (255,0,0,0)
		_SubColor("SubColor",Color)=(255,0,0,0)
				_RimLightPower("Lim Right Power", Range(0, 20.0)) = 5.0
	}
		SubShader
	{
		Tags {
...
#include "UnityCG.cginc"
	...
	float3 refract = mul(UNITY_MATRIX_VP, i.normal)*_Refraction;
		...
		fixed4 col = tex2D(_FirstGrabTexture, float4(x - refract.x, y + refract.y, i.grabPos.z, i.grabPos.w));
		...
		float3 V = normalize(i.posWS - _WorldSpaceCameraPos);
			...
			fixed3 rimColor = rim* _RimLightPower*0.05* fixed4(1,1,1,1);
			col.rgb += rimColor;
...
#include "UnityCG.cginc"
	...
	float3 refract = mul(UNITY_MATRIX_VP, i.normal)*_Refraction;
		...
		fixed4 col = tex2D(_SecondGrabTexture, float4(x - refract.x, y + refract.y, i.grabPos.z, i.grabPos.w));
